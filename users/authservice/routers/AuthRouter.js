...
router.post('/login', [
  ...
  try {
    ...
    const userResponse = await axios.get(`${gatewayServiceUrl}/users?id=${user.username}`);
    let userFromDB = userResponse.data;
    console.log('userFromDB raw:', userFromDB);

    // Handle array or object response
    if (Array.isArray(userFromDB)) {
      userFromDB = userFromDB[0]; // Take first user if array
    }
    ...
    console.log('Comparing password:', user.password, 'with hash:', userFromDB.password);
